// src/lexer/tokens.rs
use std::fmt;

#[derive(Debug, Clone, PartialEq)]
pub struct Token {
    pub kind: TokenKind,
    pub line: usize,
    pub column: usize,
    pub byte_pos: usize,
}

#[derive(Debug, Clone, PartialEq)]
pub enum TokenKind {
    Void,
    Int,
    Float,
    Bool,
    String,
    Print,
    If,
    Else,
    While,
    For,
    In,
    Return,
    Read,
    Match,
    True,
    False,
    Not,
    And,
    Or,
    Break,
    Use,
    Option,
    Raise,
    Worry,
    Empty,
    Some,
    None,
    List,
    Table,
    Char,
    Struct,
    Module,
    Mutable,
    Const,
    Enum,
    Identifier(String),
    Number(i64),
    FloatLiteral(f64),
    StringLiteral(String),
    CharLiteral(String),
    Plus,
    Minus,
    Star,
    Slash,
    Greater,
    Less,
    GreaterEqual,
    LessEqual,
    EqualEqual,
    BangEqual,
    Equals,
    PlusEquals,
    MinusEquals,
    StarEquals,
    SlashEquals,
    Percent,
    DotDot,
    Dot,
    ColonColon,
    LParen,
    RParen,
    LBrace,
    RBrace,
    Semicolon,
    Comma,
    Colon,
    Interpolation(String),
    Illegal(String),
    Eof,
}

impl fmt::Display for TokenKind {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            TokenKind::Void => write!(f, "void"),
            TokenKind::Int => write!(f, "int"),
            TokenKind::Float => write!(f, "float"),
            TokenKind::Bool => write!(f, "bool"),
            TokenKind::String => write!(f, "string"),
            TokenKind::Print => write!(f, "print"),
            TokenKind::If => write!(f, "if"),
            TokenKind::Else => write!(f, "else"),
            TokenKind::While => write!(f, "while"),
            TokenKind::For => write!(f, "for"),
            TokenKind::In => write!(f, "in"),
            TokenKind::Return => write!(f, "return"),
            TokenKind::Read => write!(f, "read"),
            TokenKind::Match => write!(f, "match"),
            TokenKind::True => write!(f, "true"),
            TokenKind::False => write!(f, "false"),
            TokenKind::Not => write!(f, "not"),
            TokenKind::And => write!(f, "and"),
            TokenKind::Or => write!(f, "or"),
            TokenKind::Break => write!(f, "break"),
            TokenKind::Use => write!(f, "use"),
            TokenKind::Option => write!(f, "option"),
            TokenKind::Raise => write!(f, "raise"),
            TokenKind::Worry => write!(f, "worry"),
            TokenKind::Empty => write!(f, "empty"),
            TokenKind::Some => write!(f, "some"),
            TokenKind::None => write!(f, "none"),
            TokenKind::List => write!(f, "list"),
            TokenKind::Table => write!(f, "table"),
            TokenKind::Char => write!(f, "char"),
            TokenKind::Struct => write!(f, "struct"),
            TokenKind::Module => write!(f, "module"),
            TokenKind::Mutable => write!(f, "mutable"),
            TokenKind::Const => write!(f, "const"),
            TokenKind::Enum => write!(f, "enum"),
            TokenKind::Identifier(s) => write!(f, "{}", s),
            TokenKind::Number(n) => write!(f, "{}", n),
            TokenKind::FloatLiteral(n) => write!(f, "{}", n),
            TokenKind::StringLiteral(s) => write!(f, "\"{}\"", s),
            TokenKind::CharLiteral(s) => write!(f, "'{}'", s),
            TokenKind::Plus => write!(f, "+"),
            TokenKind::Minus => write!(f, "-"),
            TokenKind::Star => write!(f, "*"),
            TokenKind::Slash => write!(f, "/"),
            TokenKind::Greater => write!(f, ">"),
            TokenKind::Less => write!(f, "<"),
            TokenKind::GreaterEqual => write!(f, ">="),
            TokenKind::LessEqual => write!(f, "<="),
            TokenKind::EqualEqual => write!(f, "=="),
            TokenKind::BangEqual => write!(f, "!="),
            TokenKind::Equals => write!(f, "="),
            TokenKind::PlusEquals => write!(f, "+="),
            TokenKind::MinusEquals => write!(f, "-="),
            TokenKind::StarEquals => write!(f, "*="),
            TokenKind::SlashEquals => write!(f, "/="),
            TokenKind::Percent => write!(f, "%"),
            TokenKind::DotDot => write!(f, ".."),
            TokenKind::Dot => write!(f, "."),
            TokenKind::ColonColon => write!(f, "::"),
            TokenKind::LParen => write!(f, "("),
            TokenKind::RParen => write!(f, ")"),
            TokenKind::LBrace => write!(f, "{{"),
            TokenKind::RBrace => write!(f, "}}"),
            TokenKind::Semicolon => write!(f, ";"),
            TokenKind::Comma => write!(f, ","),
            TokenKind::Colon => write!(f, ":"),
            TokenKind::Interpolation(s) => write!(f, "&{{{}}}", s),
            TokenKind::Illegal(s) => write!(f, "Illegal({})", s),
            TokenKind::Eof => write!(f, "EOF"),
        }
    }
}

impl From<crate::syntax::Keyword> for TokenKind {
    fn from(k: crate::syntax::Keyword) -> Self {
        match k {
            crate::syntax::Keyword::Int => TokenKind::Int,
            crate::syntax::Keyword::Float => TokenKind::Float,
            crate::syntax::Keyword::String => TokenKind::String,
            crate::syntax::Keyword::Bool => TokenKind::Bool,
            crate::syntax::Keyword::Void => TokenKind::Void,
            crate::syntax::Keyword::If => TokenKind::If,
            crate::syntax::Keyword::Else => TokenKind::Else,
            crate::syntax::Keyword::While => TokenKind::While,
            crate::syntax::Keyword::For => TokenKind::For,
            crate::syntax::Keyword::In => TokenKind::In,
            crate::syntax::Keyword::Print => TokenKind::Print,
            crate::syntax::Keyword::Match => TokenKind::Match,
            crate::syntax::Keyword::Return => TokenKind::Return,
            crate::syntax::Keyword::Read => TokenKind::Read,
            crate::syntax::Keyword::True => TokenKind::True,
            crate::syntax::Keyword::False => TokenKind::False,
            crate::syntax::Keyword::Not => TokenKind::Not,
            crate::syntax::Keyword::And => TokenKind::And,
            crate::syntax::Keyword::Or => TokenKind::Or,
            crate::syntax::Keyword::Break => TokenKind::Break,
            crate::syntax::Keyword::Use => TokenKind::Use,
            crate::syntax::Keyword::Option => TokenKind::Option,
            crate::syntax::Keyword::Raise => TokenKind::Raise,
            crate::syntax::Keyword::Worry => TokenKind::Worry,
            crate::syntax::Keyword::Empty => TokenKind::Empty,
            crate::syntax::Keyword::Some => TokenKind::Some,
            crate::syntax::Keyword::None => TokenKind::None,
            crate::syntax::Keyword::List => TokenKind::List,
            crate::syntax::Keyword::Table => TokenKind::Table,
            crate::syntax::Keyword::Char => TokenKind::Char,
            crate::syntax::Keyword::Struct => TokenKind::Struct,
            crate::syntax::Keyword::Module => TokenKind::Module,
            crate::syntax::Keyword::Mutable => TokenKind::Mutable,
            crate::syntax::Keyword::Const => TokenKind::Const,
            crate::syntax::Keyword::Enum => TokenKind::Enum,
        }
    }
}
